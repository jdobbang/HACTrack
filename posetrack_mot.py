...
class PoseTrackMOT(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'TRACKERS_FOLDER': os.path.join(code_path, '/home/dobbang/jeon/HACTrack/phalp_prime/trackers/'),  # Trackers location
            'PRINT_CONFIG': True,
    ...
    def __init__(self, config=None):
        ...
        for seq in self.seq_list:
            curr_file = os.path.join(self.gt_fol, seq, 'gt/gt_sampling.txt')
    ...
    def _get_seq_info(self):
        ...
        seq_lengths = dict()
        import joblib
        data_gt = joblib.load('/home/dobbang/jeon/HACTrack/phalp_prime/_DATA/posetrack_gt.pickle') 
        for seq in seq_list:

            frames = data_gt[seq].keys()
            # Convert data to required format
            seq_list_ = []
            for i in list(frames):
                seq_list_.append(int(i.split('.')[0]))
            ...
            for idx in range(len(seq_list_)):
                frame_idx = seq_list_[idx]
                image_data = image_info[frame_idx]
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        # File location
        if is_gt:
            file = os.path.join(self.gt_fol, seq, 'gt/gt_sampling.txt')
        else:
            ...
            if gt_ids.shape[0] > 0 and tracker_ids.shape[0] > 0:
                ...
                if len(match_cols) < similarity_scores.shape[1]:
                    if len(fr_ignore_regions) > 0:
                        ...
                        region_inter = np.zeros([len(fr_ignore_regions), len(unmatched_boxes)])
                        bbox_size = np.zeros([len(unmatched_boxes)])
                        for i in range(len(fr_ignore_regions)):
                            
                            for j in range(len(unmatched_boxes)):
                                
                                size = (unmatched_boxes[j].bounds[2] - unmatched_boxes[j].bounds[0])*(unmatched_boxes[j].bounds[3] - unmatched_boxes[j].bounds[1])
                                ...
                                region_inter[i, j] = poly_intersection 
                        
                        f_list = list(data_gt[raw_data['seq']])
                    
                        if len(ignore_regions) > 0 and len(unmatched_boxes) > 0:
                            ignore_candidates = np.where((region_ious > 0.1))[1].tolist()
                            
                            ignore_candidates_2 = []
                            for u_ in range(len(unmatched_boxes)):
                                temp = region_inter[:,u_] >= 0.8*bbox_size[u_]
                                if True in temp:
                                    ignore_candidates_2.append(u_)
                            ignore_candidates = list(set(ignore_candidates + ignore_candidates_2))
                            """
                            #visualizew
                            for u in range(len(unmatched_boxes)):
                                cv2.rectangle(img,(int(unmatched_boxes[u].bounds[0]),int(unmatched_boxes[u].bounds[1])),(int(unmatched_boxes[u].bounds[2]),int(unmatched_boxes[u].bounds[3])),(0,0,255),2)

                            for i_r in range(len(fr_ignore_regions)):
                                pts = []
                                for xy in range(len(fr_ignore_regions[i_r].exterior.xy[0])):
                                    x,y = fr_ignore_regions[i_r].exterior.xy[0],fr_ignore_regions[i_r].exterior.xy[1]
                                    pts.append([int(x[xy]),int(y[xy])])
                                pts_ = np.array(pts)
                                cv2.polylines(img,[pts_],True,(255,0,255),2)

                            #cv2.imwrite('/home/dobbang/jeon/4dhumans/eval/'+seq+'/'+f_list[t],img)
                            """
                        if len(ignore_candidates) > 0:
                            dets_to_remove = [] 
                            for remove_idx in ignore_candidates:
                                #cv2.rectangle(img,(int(unmatched_boxes[remove_idx].bounds[0]),int(unmatched_boxes[remove_idx].bounds[1])),(int(unmatched_boxes[remove_idx].bounds[2]),int(unmatched_boxes[remove_idx].bounds[3])),(255,255,255),4)
                                #cv2.imwrite('/home/dobbang/jeon/4dhumans/eval/'+seq+'/'+f_list[t],img)
