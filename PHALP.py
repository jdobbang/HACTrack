...
from phalp.external.deep_sort_.tracker import Tracker, ByteTracker, HACTracker
...
class PHALP(nn.Module):
    ...
    def setup_deepsort(self):
        ...
        self.tracker = HACTracker(self.cfg, metric, max_age=self.cfg.phalp.max_age_track, n_init=self.cfg.phalp.n_init, phalp_tracker=self, dims=[4096, 4096, 99])  
    ...
    def track(self):
        ...
        self.setup_deepsort()
        self.default_setup()
        log.info("Saving tracks at : " + self.cfg.video.output_dir)
        ...
        try: 
            ...
            for t_, frame_name in progress_bar(enumerate(list_of_frames), description="Tracking : " + self.cfg.video_seq, total=len(list_of_frames), disable=False):
                ...
                with open('/home/dobbang/jeon/4dhumans/detections/'+self.cfg.video_seq+'/json/new_bbox.json','r') as f1:
                    new_bbox = json.load(f1)

                ############ detection ##############
                pred_bbox, pred_masks, pred_scores, pred_classes, gt_tids, gt_annots = self.get_detections(new_bbox,image_frame, frame_name, t_, additional_data, measurments)
    ...
    def get_detections(self, new_detection, image, frame_name, t_, additional_data=None, measurments=None):
        """
        ...
        if 1:
            ...
            new_DH = new_detection[frame_name.split('/')[-1]]["DH"]
            new_DL = new_detection[frame_name.split('/')[-1]]["DL"]
